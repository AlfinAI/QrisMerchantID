...
def test_expired_access_token_refreshes_once_and_retries():
    script = ScriptedTransport(
        [
            (401, {"errors": [{"message": "Invalid/Expired token"}]}),
            (201, {"access_token": "NEW", "refresh_token": "NEW-REF"}),
            (200, {"transactions": []}),
        ]
    )
    client = GoPayClient(access_token="OLD", refresh_token="REF", transport=script)
    assert client.get("/merchant-analytics/v2/merchants/transactions") == {"transactions": []}
    assert client.api_headers()["Authorization"] == "Bearer NEW"
    assert client.refresh_token == "NEW-REF"
    assert len(script.calls) == 3


def test_success_false_expired_token_refreshes_once_and_retries():
    script = ScriptedTransport(
        [
            (200, {"success": False, "error": "Invalid/Expired token"}),
            (201, {"access_token": "NEW", "refresh_token": "NEW-REF"}),
            (200, {"transactions": []}),
        ]
    )
