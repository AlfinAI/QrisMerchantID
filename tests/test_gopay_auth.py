...
def test_refresh_session_uses_verified_grant_and_rotates_tokens():
    fake = FakeHttpClient()
    fake.add_route("POST", "/goid/token", 201, {"access_token": "NEW", "refresh_token": "NEW-REF"})
    gopay = GoPayMerchant(access_token="OLD", refresh_token="OLD-REF", transport=fake)
    session = gopay.auth.refresh_session()
    assert session == {"access_token": "NEW", "refresh_token": "NEW-REF"}
    assert gopay.client.api_headers()["Authorization"] == "Bearer NEW"
    assert gopay.client.refresh_token == "NEW-REF"
    assert json.loads(fake.calls[0]["body"]) == {
        "client_id": "go-biz-web-new",
        "grant_type": "refresh_token",
        "data": {"refresh_token": "OLD-REF"},
    }


def test_refresh_session_requires_cached_refresh_token():
    with pytest.raises(ValueError, match="refresh_token"):
        GoPayMerchant(access_token="OLD", transport=FakeHttpClient()).auth.refresh_session()
