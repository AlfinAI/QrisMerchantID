...
- GoPay: verified refresh-token session recovery. The client retains both tokens,
  refreshes once on the captured `Invalid/Expired token` response or HTTP 401/403
  authentication response, retries the original request, and stores the rotated
  refresh token. Offline tests cover HTTP and JSON auth-expiry paths.
- GoPay documentation: documented the verified `/goid/token` contract, secure session
  persistence, the explicit re-login fallback, and the limits of the evidence.
