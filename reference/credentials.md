...
> Aturan: tidak ada nilai mentah di file ini. Format masking:
> `prefix…suffix (len=N)`.
...
Kesimpulan: tidak ada static secret kriptografis di client — konsisten dengan
arsitektur server-side signing (token diterbitkan server, GoID SSO).
Konfirmasi runtime: TODO-DYN-1 (keduanya).
