...
When `refresh_token` is configured, the client automatically performs one verified
refresh-token exchange when GoBiz reports an expired token, then retries the original
request with the new access token. The rotated refresh token must be saved again:

```python
new_session = gopay.auth.refresh_session()  # optional explicit refresh
token_cache.save(".gopay-session.json", new_session)
```

This avoids repeated logout/login cycles during normal access-token expiry. It does **not**
guarantee a permanent session: GoBiz may revoke or expire the refresh token, change the
account state, or reject the request. In those cases the library stops retrying and a fresh
login is required. The captured portal evidence verifies the endpoint and payload; GoPay's
account-suspension policy and the claim that repeated login alone causes suspension are
**BELUM TERVERIFIKASI**. Keep polling conservative (the recommended 6-second interval).

See [examples/01_login_otp.py](../../examples/01_login_otp.py) and
