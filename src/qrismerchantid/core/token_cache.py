...
GoBiz sessions carry server-side expiry even though the ``/goid/token``
response does not expose an ``expires_in`` field. The verified portal flow
returns ``access_token`` and ``refresh_token``; callers should persist both and
exchange the refresh token before re-login. A missing ``expires_at`` means the
server, not a guessed local TTL, decides validity. When callers do know an
expiry (e.g. the ``otp_expires_in`` window, or a self-imposed TTL), they store
it as an absolute epoch in ``expires_at`` and :func:`load` enforces it.
"""
